...
import { TableColumnTypes, TablePaginationTypes, TablePropTypes } from './Table.types'
  ...
  return (
    <Card>
      <CardHeader title={settings.title}>
          ...
          settings.bulkActions?.map((action) => (
            <Button
              ...
              onClick={() => {
                ...
                settings.bulkActionEvent && settings.bulkActionEvent(action.action, selectedRowKeys)
              }}
              key={action.action}
            >
              {action.title}
        ...
        <Dropdown overlay={columnPicker} trigger={['click']} placement="bottomRight" arrow>
                ...
                <>
                  {(() => {
                    switch (col?.type) {
                      ...
                      case 'date':
                        return moment(value && value.replace('Z', '+06:00')).format('DD-MM-YYYY')
                      case 'dateTime':
                        return moment(value && value.replace('Z', '+06:00')).format('hh:mm A DD-MM-YYYY')
                      case 'number':
                        return value ? value : 0
                      case 'decimal':
                        return value ? parseFloat(value).toFixed(2) : '0.00'
                      case 'currency':
                        return value ? parseFloat(value).toFixed(2) + ' BDT' : '0.00 BDT'
                      case 'percentage':
                        return value ? parseFloat(value) + '%' : '0%'
                      case 'image':
                        return value ? <img src={value} width={100} height={100} /> : 'No Image'
                      case 'color':
                        return (
                          <>
                            {value &&
                ...
                {settings.rowActions?.map((action, i) => (
                  <Tooltip title={action.title} key={i}>
                    {action?.action === 'delete' ? (
                      <Popconfirm
                        title={action?.rowActionTitle || 'Are you sure to delete?'}
                        onConfirm={() => {
                          // eslint-disable-next-line @typescript-eslint/no-unused-expressions
                          settings.rowActionEvent && settings.rowActionEvent(action.action, record)
                        }}
                        okText="Yes"
                        cancelText="No"
                      >
                        <Button
                          ...
                          size="small"
                          key={action.action}
                          icon={action?.btnIcon && action?.btnIcon?.type?.render()}
                          danger={action.isDanger}
                        />
                      </Popconfirm>
                    ) : action?.action === 'save' ? (
                      <Popconfirm
                        title={action?.rowActionTitle || 'Are you sure to save?'}
                        onConfirm={() => {
                          // eslint-disable-next-line @typescript-eslint/no-unused-expressions
                          settings.rowActionEvent && settings.rowActionEvent(action.action, record)
                        }}
